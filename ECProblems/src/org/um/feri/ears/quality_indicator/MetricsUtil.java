...
public class MetricsUtil {
   ...
   */
  public MOParetoIndividual readNonDominatedSolutionSet(String path) {
    try {
      /* Open the file */
      FileInputStream fis   = new FileInputStream(path)     ;
      InputStreamReader isr = new InputStreamReader(fis)    ;
      ...
      MOParetoIndividual solutionSet = new NonDominatedSolutionList();
      
      String aux = br.readLine();
      while (aux!= null) {
        StringTokenizer st = new StringTokenizer(aux);
        int i = 0;
        MOIndividual solution = new MOIndividual(st.countTokens());
        while (st.hasMoreTokens()) {
          double value = new Double(st.nextToken());
	...
	public void readNonDominatedSolutionSet(String path, NonDominatedSolutionList solutionSet) {
	   ...
	   **/
	  public double[] hvContributions(MOParetoIndividual [] populations) {
		  boolean empty = true;
      ...
      for (MOParetoIndividual population1 : populations) size += population1.size();
	   ...
	   * @return HV contributions of each population
	   **/
	  public double[] hvContributions(MOParetoIndividual archive, MOParetoIndividual [] populations) {
		  ...
		  //determining the global size of the population
      for (MOParetoIndividual population : populations) size += population.size();
