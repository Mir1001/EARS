...
public class GSA extends Algorithm {
	...
	public GSA(int pop_size, int  final_per) {
        ...
        ai = new AlgorithmInfo("GSA","@article{Rashedi20092232,"+
...
"GSA","Matlab GSA");  //EARS add algorithm name
	...
	@Override
	public Individual run(Task t) throws StopCriteriaException {
		initPopulation(t);
		int iteration = 1;
		...
		for (int i=0; i<pop_size; i++) {
			...
			for (int ii=0; ii<kbest; ii++) { //mass for best k
				if (ii!=i) {
					...
					tmp = 0;
					for (int d=0; d<D; d++) {
						tm = pop_x.get(i).getX()[d]-pop_x.get(ii).getX()[d];
						tmp+=tm*tm;
					}
					R = Math.sqrt(tmp);
					...
					for (int d=0; d<D; d++) {
						// E(i,k)=E(i,k)+rand*(M(j))*((X(j,k)-X(i,k))/(R^Rpower+eps));
						pop_x.get(i).getE()[d] +=Util.rnd.nextDouble()*pop_x.get(ii).getMass()*(pop_x.get(ii).getX()[d]-pop_x.get(i).getX()[d])/(Math.pow(R,Rpower)+eps);
					}
				}
			}
			for (int d=0; d<D; d++) {
